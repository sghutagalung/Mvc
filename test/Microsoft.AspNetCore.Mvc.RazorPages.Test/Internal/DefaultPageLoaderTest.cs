...
namespace Microsoft.AspNetCore.Mvc.RazorPages.Internal
{
    public class DefaultPageLoaderTest
    {
        ...
        public void CreateDescriptor_EmptyPageModel()
        {
            ...
            Assert.Empty(result.BoundProperties);
            Assert.Empty(result.HandlerMethods);
            Assert.Same(typeof(EmptyPageModel).GetTypeInfo(), result.HandlerTypeInfo);
        ...
        public void CreateDescriptor_FindsHandlerMethod_OnPage()
        {
            // Arrange
        ...
        public void CreateHandlerMethods_ParsesMethod()
        {
            ...
            Assert.Collection(
                ...
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnGet)), handler.MethodInfo);
                    Assert.Equal("Get", handler.HttpMethod);
                    Assert.Equal(0, handler.FormAction.Length);
                },
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnPutAdd)), handler.MethodInfo);
                    Assert.Equal("Put", handler.HttpMethod);
                    Assert.Equal("Add", handler.FormAction.ToString());
                },
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnPostAddCustomer)), handler.MethodInfo);
                    Assert.Equal("Post", handler.HttpMethod);
                    Assert.Equal("AddCustomer", handler.FormAction.ToString());
                },
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnDeleteDeleteAsync)), handler.MethodInfo);
                    Assert.Equal("Delete", handler.HttpMethod);
                    Assert.Equal("Delete", handler.FormAction.ToString());
                },
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnZAsync)), handler.MethodInfo);
                    Assert.Equal("Z", handler.HttpMethod);
                    Assert.Null(null);
                },
                handler =>
                {
                    Assert.Same(type.GetMethod(nameof(PageModelWithFormActions.OnZZ)), handler.MethodInfo);
                    Assert.Equal("Z", handler.HttpMethod);
                    Assert.Equal("Z", handler.FormAction);
                });
        }
        ...
        private class PageModelWithFormActions
        {
            ...
            public void OnDeleteDeleteAsync()
            {
            }

            public void OnZAsync() { }
